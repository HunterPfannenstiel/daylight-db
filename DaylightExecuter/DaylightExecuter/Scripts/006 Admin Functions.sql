...
BEGIN
	RETURN QUERY
	SELECT * FROM
	(SELECT json_agg(json_build_object('id', G.grouping_id, 'name', G.name)) AS groupings FROM store.grouping G) G
	CROSS JOIN (
		SELECT json_agg(EG) AS extra_groupings FROM (SELECT EC.name, EC.extra_category_id AS "id", json_agg(json_build_object('name', EG.name, 'id', EG.extra_group_id, 
	...
	CROSS JOIN (
		...
		FROM (SELECT IC.name, IC.item_category_id AS "id", array_agg(json_strip_nulls(json_build_object('name', ISC.name, 'id', ISC.item_subcategory_id))) AS entities
...
RETURNS TABLE (intital_groupings JSON)
SECURITY DEFINER
LANGUAGE plpgsql
AS
$func$
BEGIN
	RETURN QUERY
	SELECT json_object_agg(EC.extra_category_id, IEG.extra_group_id)
